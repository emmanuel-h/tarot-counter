...
[ Main Menu ]  [ New Game ]  [ Back to game ]
  text btn        filled         outlined
```
...
| **Main Menu** | `TextButton` (bottom-left) | Navigates to the landing screen. Tertiary action. |
| **New Game** | `Button` (bottom-centre, primary) | Navigates to the setup screen. All game state is discarded. |
| **Back to Game** | `OutlinedButton` (bottom-right) | Same as the back arrow — resumes the current game. |

All three bottom buttons appear on the same horizontal line with equal widths (`Modifier.weight(1f)`) and an 8 dp gap between them (`Arrangement.spacedBy`). This ensures they fit on all supported screen sizes (min SDK 24, down to ~360 dp wide) without overflow.
